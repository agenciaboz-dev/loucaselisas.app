import React from "react"
import { Avatar, Box, Button, Typography, useMediaQuery } from "@mui/material"
import destaque from "../assets/login/Frame747.svg"
import logoMarca from "../assets/login/logo_wide.svg"
import logo from "../assets/login/Vector.svg"
import WhatsAppIcon from "@mui/icons-material/WhatsApp"
import InstagramIcon from "@mui/icons-material/Instagram"
import { FaFacebookF } from "react-icons/fa"
import { MenuHome } from "../components/MenuHome"
import { paths } from "../paths"
import { MenuMobileDrop } from "../components/MenuMobileDrop"
import appstorebutton from "../assets/App Store.svg"
import googleplaybutton from "../assets/Google Play.svg"

interface HomeProps {}

export const Home: React.FC<HomeProps> = ({}) => {
    const isMobile = useMediaQuery("(orientation:portrait")

    const socialUrls = ["https://wa.me/5548999401049", "https://www.instagram.com/loucaselisas/", "https://www.facebook.com/LoucasLisas/"]

    return (
        <Box sx={{ width: 1, height: "100vh", py: "2vh" }}>
            <Box
                sx={{
                    width: "75vw",
                    height: 1,
                    margin: "0 auto",
                    padding: "5vw 0",
                    display: "flex",
                    flexDirection: "column",
                    justifyContent: "space-between",
                }}
            >
                <Box sx={{ justifyContent: "space-between", gap: isMobile ? "0.5vw" : "1.5vw", alignItems: "center" }}>
                    <Box flex={isMobile ? 0.9 : 0.2}>
                        <Avatar
                            variant="square"
                            src={logoMarca}
                            sx={{ height: isMobile ? "10vw" : 1, width: isMobile ? "auto" : 1, objectFit: "contain" }}
                        />
                    </Box>
                    {isMobile ? (
                        <MenuMobileDrop />
                    ) : (
                        <Box flex={0.7} sx={{ justifyContent: "end", gap: "1.8vw" }}>
                            {paths.map((path) => (
                                <MenuHome key={path.link} title={path.title} link={path.link} />
                            ))}
                        </Box>
                    )}
                </Box>

                <Box
                    sx={{
                        width: isMobile ? "100%" : "50%",
                        flexDirection: "column",
                        margin: "0 auto",
                        alignItems: "center",
                        gap: isMobile ? "5vw" : "1.5vw",
                    }}
                >
                    <Avatar
                        variant="square"
                        src={destaque}
                        sx={{
                            width: 1,
                            height: "auto",
                            objectFit: "fill",
                        }}
                    />
                    <Typography
                        variant="h1"
                        sx={{
                            fontSize: "1.5rem",
                            fontWeight: "700",
                            color: "white",
                        }}
                    >
                        Baixe o App:
                    </Typography>
                    <Box sx={{ gap: isMobile ? "5vw" : "1vw", flexDirection: isMobile ? "column" : "row" }}>
                        <Button
                            variant="contained"
                            color="primary"
                            sx={{ height: isMobile ? "18vw" : "4vw", width: "auto", padding: 0, borderRadius: isMobile ? "3vw" : "0.5vw" }}
                        >
                            <img src={appstorebutton} alt="App Store" style={{ height: isMobile ? "18vw" : "4vw", width: "100%" }} />
                        </Button>
                        <Button
                            variant="contained"
                            color="primary"
                            sx={{ height: isMobile ? "18vw" : "4vw", width: "auto", padding: 0, borderRadius: isMobile ? "3vw" : "0.5vw" }}
                        >
                            <img src={googleplaybutton} alt="Google Play" style={{ height: isMobile ? "18vw" : "4vw", width: "100%" }} />
                        </Button>
                    </Box>
                </Box>

                <Box
                    sx={{
                        flexDirection: isMobile ? "column" : "row",
                        justifyContent: "space-between",
                        gap: isMobile ? "3vw" : "",
                        alignItems: isMobile ? "center" : "",
                    }}
                >
                    <Box gap={isMobile ? "4vw" : "1.5vw"}>
                        <Button
                            variant="contained"
                            color="secondary"
                            sx={{ p: isMobile ? "2vw" : "0.5vw", borderRadius: "30vw", minWidth: 0 }}
                            onClick={() => window.open(socialUrls[0], "_blank")}
                        >
                            <WhatsAppIcon sx={{ fontSize: isMobile ? "1.5rem" : "1.2rem" }} />
                        </Button>
                        <Button
                            variant="contained"
                            color="secondary"
                            sx={{ p: isMobile ? "2vw" : "0.5vw", borderRadius: "30vw", minWidth: 0 }}
                            onClick={() => window.open(socialUrls[1], "_blank")}
                        >
                            <InstagramIcon sx={{ fontSize: isMobile ? "1.5rem" : "1.2rem" }} />
                        </Button>
                        <Button
                            variant="contained"
                            color="secondary"
                            sx={{ p: isMobile ? "2.5vw" : "0.5vw", borderRadius: "30vw", minWidth: 0 }}
                            onClick={() => window.open(socialUrls[2], "_blank")}
                        >
                            <FaFacebookF style={{ fontSize: "1.2rem" }} />
                        </Button>
                    </Box>
                    <Box gap="1vw" sx={{ color: "secondary.main" }}>
                        <Avatar src={logo} sx={{ display: isMobile ? "none" : "", height: "1.5rem", width: "1.5rem" }} />
                        <Typography variant="body1">© 2024 Loucas & Lisas</Typography>
                    </Box>
                </Box>
            </Box>
        </Box>
    )
}
